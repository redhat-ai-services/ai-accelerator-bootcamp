...
. The next file, `03_outputs_pipeline.py`, introduces the output capability. Run it in the notebook terminal with the 'python pipelines/03_outputs_pipeline.py' command and check the output in the Runs section in RHOAI
...
. The `08_additional_packages_pipeline.py` shows how we can install additional packages on top of the base image through the `packages_to_install` feature of kfp. This is a great feature for experimentation. Run it with `python pipelines/08_additional_packages_pipeline.py` and check the output in the Runs section.
